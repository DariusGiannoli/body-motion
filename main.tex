...
different onboard viewpoints~\cite{ben_focaldrone}. Their transmitter mapped
...
In both conditions, participants to viewed  the FPV through a virtual reality (VR) headset (Meta Quest 3S, Meta, USA). Body motion was measured using the headset for head yaw, two handheld controllers for hand positions, and a chest-mounted inertial measurement unit (LPMS-B2, LP-Research, Japan) measured torso inclination. The swarm controller, command limits, focal-agent logic, and simulation were the same for both conditions. Only the input hardware and filtering differed.
...
adjustment, and formation-spacing control. Each course included one 90-degree turn, requiring participants to rotate the FPV and continue in the rotated
focal-agent reference frame. Stars distributed across the gate openings encouraged control of swarm extent rather than only the focal agent or centroid; collection yield measured this collective performance.
...
Path directness used the endpoint-distance-to-path-length definition of path
...
gates, and from the final gate to the finish, using the measured segment
endpoints.
...
travel decreased by 44.0\%, corresponding to 16.9~m (Holm-adjusted $p=0.049$) while excess horizontal travel was 14.2\% lower with the implemented body-motion interface (48.8 versus 56.8~m), but no clear difference was detected (mean difference $-8.0$~m, $p=0.153$). Figure~\ref{fig:trajectory_overlays} illustrates these patterns.
...
While overall workload and usability were similar between conditions, the implemented body-motion interface required significantly more physical effort. We observed no statistical difference in the overall weighted TLX score (mean difference 7.2; Holm-adjusted $p=0.482$) or the SUS usability score (mean difference 1.6; Holm-adjusted $p=0.491$). However, all 14 participants reported higher physical demand with the implemented body-motion interface, resulting in a mean increase of 40.7 points (Holm-adjusted $p<0.001$).  Interface preference was divided, and relative motion-sickness responses were
mixed (Fig.~\ref{fig:trajectory_overlays}).
...
\section{Discussion}
The implemented body-motion interface improved traversal efficiency and path directness in the tested FPV swarm-navigation task, with the trajectory
...
\cite{jacob1994integrality,zhai1998quantifying}. Second, body motion reduced delivered-command variation, which may have supported more continuous forward motion and fewer corrections. These associations are consistent with the measured efficiency gains but do not establish causation.
...
Fig.~\ref{fig:trajectory_overlays}(b) are consistent with repeated altitude
corrections under transmitter control, although the figure alone does not
establish their cause. The fixed throttle gain may have been too sensitive for
some participants. In addition, the body-motion altitude command was
...
The body-motion interface required more physical effort, although overall
