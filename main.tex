...
Body-motion control improved traversal in the tested task. Participants
finished 19.4\% faster, and 13 of 14 were faster with body motion. Their paths
were 7.0\% shorter, and their mean speed was 12.5\% higher. They also spent less
time moving slowly, followed more direct paths, and made less excess vertical
travel. These results indicate more continuous and direct movement. However,
path length and mean speed are related to completion time. They support the
main result but do not explain its cause.

Delivered commands varied 88.8\% less with body motion. The difference remained
large after the same extra offline filter was applied to both command streams.
Both interfaces provided the same five controls. However, they used different
hardware, calibration, mappings, and online filters. The results therefore
compare the two complete systems. They do not show which difference caused the
faster traversal.

The study found no clear differences in gate centering, collection yield, or
swarm survival. This does not show that the interfaces had the same task
performance or safety. The sample was small, and survival was almost 100\% with
both interfaces. Body motion also increased physical demand by 40.7 points,
and every participant reported higher physical demand. Total TLX and SUS scores
showed no clear differences. Eight participants preferred body motion, five
preferred the transmitter, and one had no preference.

The physical cost suggests several design changes. Arm support may reduce
effort~\cite{rognon2018flyjacket}. Higher control gains could reduce the
required movement, but they may also reduce precision. Participant comments
also suggest that the interface should correct changes in hand height when the
torso moves. Feedback about formation spacing may improve spatial awareness.
Tactile feedback is one option~\cite{tsykunov2019swarmtouch}. These changes
require testing.
...
kratky2025gesture} to rear-focal FPV swarm control. It also complements earlier
transmitter-based FPV control~\cite{ben_focaldrone}. Direct comparison is not
possible because the studies used different tasks, viewpoints, swarm sizes,
and measures.

The main limits were the small sample, the lack of a power analysis, the short
exposure, and the fixed course order. The study also tested only one simulated
15-agent swarm. Future studies should use larger samples, vary the course order,
provide more training, and use harder tasks. They should record commands before
and after processing and compare matched mappings and filters. Tests with
physical swarms are also needed to measure the effects of communication delays,
aerodynamics, and real safety constraints.
