...
{online video playlist (clickable link)}.}%
...
Consistent with task-oriented HRI evaluation~\cite{steinfeld2006metrics},
completion time was the primary outcome; the remaining measures characterized
traversal and trajectory behavior, task performance and swarm integrity,
delivered-command behavior, and subjective experience.

\emph{Traversal efficiency.}
Completion time $T$ spanned the recorded start and finish events. Centroid path
length $L$ was the cumulative three-dimensional distance traveled by the
controlled-component centroid, and mean speed was $\bar v=L/T$. Because these
measures are mathematically related, path length and mean speed were treated as
supporting rather than independent outcomes.

\emph{Trajectory characteristics.}
Movement continuity was measured as the fraction of traversal time with
centroid speed below $0.5~\mathrm{m\,s^{-1}}$. Using the
endpoint-distance-to-path-length definition of path
efficiency~\cite{mavrogiannis2022socialmomentum}, the segment-aggregated
path-directness ratio was
\begin{equation}
D_{\mathrm{path}}
=
\frac{1}{L}
...
\end{equation}
where $\mathbf{x}_{j,\mathrm{start}}$ and $\mathbf{x}_{j,\mathrm{end}}$ are
the centroid positions at the endpoints of the eight start--gate, gate--gate,
and gate--finish segments. Higher values indicate more direct paths. Because
the endpoints were realized rather than nominal, $D_{\mathrm{path}}$ measures
overall directness, not centerline adherence or whether inefficiency arose from
deviation, curvature, or reversal.

Excess vertical travel, $V_{\mathrm{exc}}$, was the sum over segments of the
difference between total absolute vertical displacement and the absolute
endpoint-to-endpoint height change. Lower low-speed fraction and
$V_{\mathrm{exc}}$, and higher $D_{\mathrm{path}}$, indicated more continuous
and direct movement.

\emph{Task performance and swarm integrity.}
Gate-centering error was the mean, across the first forward crossings of the
seven gates, of the larger of $|h_g|/(W_g/2)$ and $|v_g|/(H_g/2)$, where
$h_g$ and $v_g$ are the horizontal and vertical offsets and $W_g$ and $H_g$
are the opening dimensions. Values of zero and one denote the opening center
and boundary, respectively; values greater than one indicate a crossing
outside the opening.

Collection yield was the fraction of the 316 available stars collected. Swarm
survival was $1-N_{\mathrm{crash}}/15$, where $N_{\mathrm{crash}}$ is the
number of unique crashed agents. Largest-component retention was the
time-weighted fraction of surviving agents in the largest connected component.
...
$\mathbf{q}_k\in[0,1]^{5}$. For $M$ samples spanning $\tau=t_M-t_1$, with
...
interfaces used different online filters, $E_{\mathrm{cmd}}$ characterizes the
complete delivered-command pipelines rather than intrinsic input smoothness.

\emph{Subjective experience.}
Weighted NASA-TLX and its six dimensions assessed
workload~\cite{hart1988nasatlx}, while SUS assessed
usability~\cite{brooke1996sus}. Raw TLX was used as a scoring sensitivity
check; interface preference and relative motion sickness were summarized
descriptively.

\subsection{Data Processing and Statistical Analysis}
\label{sec:statistical_analysis}

All 56 runs were retained. Metrics were computed from the native 15-Hz samples
without additional smoothing or resampling. The two runs were averaged within
participant and interface, yielding 14 paired observations.

Interfaces were compared using exact two-sided Wilcoxon signed-rank
tests~\cite{wilcoxon1945ranking} ($\alpha=.05$). Holm
correction~\cite{holm1979procedure} was applied separately within the
traversal, trajectory, task and swarm-integrity, global-subjective, and
TLX-dimension families; the single delivered-command outcome required no
adjustment. Body-minus-transmitter mean differences are reported with
paired-bootstrap 95\% confidence intervals based on 20,000
resamples~\cite{efron1993bootstrap}. Sensitivity analyses examined course,
condition order, path sampling, common filtering, and Raw TLX scoring.

% =====================================================================
\section{Results}

All 14 participants were included in the paired analyses.
\begin{figure}[!tb]
\centering
...
\emph{Traversal efficiency.}
Completion time was 19.4\% lower with body motion
(93.0 versus 115.4~s), corresponding to a mean reduction of
22.4~s (95\% CI $[13.4,31.8]$; Holm-adjusted $p=.001$).
Thirteen of the 14 participants were faster with body motion
(Fig.~\ref{fig:paired_results}(a)). Paths were 7.0\% shorter
(298.6 versus 321.1~m; Holm-adjusted $p=.011$), while mean speed was
12.5\% higher
(3.27 versus 2.91~$\mathrm{m\,s^{-1}}$; Holm-adjusted $p=.001$).

\emph{Trajectory characteristics.}
The fraction of traversal time below $0.5~\mathrm{m\,s^{-1}}$ was lower with
body motion (2.5\% versus 10.6\%), corresponding to a mean reduction of
8.1 percentage points (95\% CI $[5.8,10.2]$; Holm-adjusted $p<.001$).
The path-directness ratio was higher with body motion
(0.776 versus 0.725), with a mean increase of 0.052
(95\% CI $[0.021,0.082]$; Holm-adjusted $p=.021$;
Fig.~\ref{fig:paired_results}(b)).
Excess vertical travel was 44.0\% lower
(21.6 versus 38.5~m), corresponding to a mean reduction of
16.9~m (95\% CI $[4.9,30.1]$; Holm-adjusted $p=.025$;
Fig.~\ref{fig:paired_results}(c)).
Figure~\ref{fig:trajectory_overlays} shows the corresponding horizontal paths
and height profiles.
...
\emph{Delivered-command behavior.}
Delivered-command variation was 88.8\% lower with the implemented body-motion
pipeline ($p<.001$). Applying the same additional 0.143-s low-pass filter to
both delivered streams preserved an approximately 80\% reduction. Because
pre-filter body commands were not logged, this comparison characterizes the
complete pipelines rather than input modality alone.

\emph{Task performance and swarm integrity.}
No clear interface differences were detected in gate-centering error,
collection yield, swarm survival, or largest-component retention
(all Holm-adjusted $p=1.000$). Survival and largest-component retention
remained near ceiling with both interfaces; these findings do not establish
equivalent task performance or safety.

\emph{Subjective experience.}
Weighted TLX did not clearly differ between body motion and the transmitter
(59.1 versus 51.9; mean difference $=7.2$; 95\% CI $[-5.5,20.2]$;
Holm-adjusted $p=.482$), nor did SUS
(60.7 versus 59.1; mean difference $=1.6$; 95\% CI $[-12.1,14.1]$;
Holm-adjusted $p=.491$). Physical demand was 40.7 points higher with body
motion (63.7 versus 23.0; 95\% CI $[30.7,50.4]$;
Holm-adjusted $p<.001$) and was higher for all 14 participants
(Fig.~\ref{fig:paired_results}(d)). Eight participants preferred body motion,
five preferred the transmitter, and one reported no preference.
Motion-sickness reports were mixed.
