...
\caption{Centroid trajectories for both courses. Panels (a,b) show Course~1,
and panels (c,d) show Course~2. The left and right columns show top views and
gate-aligned height profiles, respectively. Thin curves are individual runs,
thick curves are interface medians, and dashed lines indicate the mean run-wise
maximum height.}
...
% {Figures/fig_trajectory_overlays_height_mean_max_1x4.pdf}
...
Body-motion control reduced completion time by 19.4\%, with shorter trajectories
and less time at low speed. The higher path-directness ratio indicates that the
trajectories more closely followed the straight-line connections between
successive course landmarks. Excess vertical travel was also 44.0\% lower,
indicating less unnecessary ascent and descent. Because gate-centering error
did not clearly differ, this finding reflects more economical vertical movement
rather than greater positional accuracy. Together, these measures suggest that
the efficiency advantage was expressed through more continuous and direct
movement. This gain was not accompanied by a detected trade-off in collection
yield or swarm survival, suggesting that faster traversal did not come at an
observable cost to these task outcomes. However, the study did not establish
equivalent performance or safety.

At the command level, body motion produced a substantially steadier control
stream: delivered-command variation was 88.8\% lower and remained 80.0\% lower
after common offline filtering. Fewer abrupt changes in the reference commands
could help sustain motion and reduce corrective maneuvers. This interpretation
is consistent with the lower low-speed fraction, higher path directness, and
lower excess vertical travel, although the analysis does not establish a causal
relationship.

Overall TLX and SUS scores did not clearly differ between the interfaces.
However, physical demand was 40.7 points higher with body motion, and all
participants rated it as more physically demanding. This result is consistent
with the required arm and torso movements. The lower ratings for body motion on most dimensions partly offset its higher physical demand. Eight participants preferred body motion, five preferred the transmitter, and one had no preference. Motion-sickness reports did not clearly favor either interface.

The higher physical demand and repeated requests for arm support suggest two
design improvements. Arm support, as used in prior body-based drone
control~\cite{rognon2018flyjacket}, could reduce sustained effort. Higher
shoulder-motion gains could reduce the required movement range, but may make
...
macchini2024datadriven} to multi-agent swarm control, and from externally
viewed formations~\cite{macchini2021personalized,tsykunov2019swarmtouch,
kratky2025gesture} to rear-focal FPV operation. It also complements earlier
transmitter-based FPV control~\cite{ben_focaldrone} by evaluating body motion
as an alternative input method.

Future studies should use larger samples, provide longer training, and include
more demanding tasks, such as time-limited navigation, dynamic obstacles, or
competing objectives. Experiments with physical swarms are also needed to
assess communication delays, aerodynamic effects, and real-world safety
constraints.
