...
A conventional radio-control transmitter (FrSky Taranis X9D Plus) generated the same command vector, $\mathbf{u}_t$, as the body-motion interface. As shown in ~\ref{fig:overview}, the right stick controlled planar
...
The gate-centering error was calculated by dividing the horizontal offset by half the gate width and the vertical offset by half the gate height. The larger of these two normalized values was selected for each gate, and these maximums were then averaged across all seven gates. A final value of zero indicates a perfectly centered crossing, while a value of one or above one indicates a crossing at or outside the boundary.
...
\caption{Run-level performance by condition ($n = 14$; $*p < 0.05$, $**p < 0.01$, $***p < 0.001$): completion time, path length, mean speed, collection \%, crashes, disconnections, control command smoothness, gap-center deviation, and multi-DOF coordination.}
...
Body-motion control improved overall efficiency by enabling more continuous and direct movement. It reduced completion time by 19.4\% through shorter trajectories and less time spent at low speeds. Higher path directness showed that the trajectories more closely followed straight lines between landmarks. Additionally, a 44.0\% reduction in excess vertical travel indicated fewer unnecessary ascents and descents. Because gate-centering error remained unchanged, this reduction reflects more economical movement rather than greater positional accuracy. Importantly, this faster traversal did not compromise collection yield or swarm survival, though this lack of a detected trade-off does not strictly prove equivalent safety.
