...
\caption{Exploratory decomposition of delivered-command co-change.}
\label{fig:trajectory_overlays}
\end{figure}
...
The trajectory difference was concentrated in the vertical component. The
larger vertical reversals illustrated in Fig.~\ref{fig:trajectory_overlays}(b)
are consistent with repeated altitude corrections under transmitter control,
although the figure alone does not establish their cause. One possibility is
that the fixed throttle gain was too sensitive for some participants. Moreover,
the body-motion altitude command was individually calibrated and smoothed,
whereas the transmitter command was neither personalized nor additionally
filtered. The observed difference therefore characterizes the complete
implemented interfaces and cannot be attributed solely to input modality.
Future comparisons should match temporal filtering and evaluate
participant-specific transmitter gains.
...
me, the increased multi-DOF coordination may expand the complexity of environments that a single operator can negotiate. By allowing the swarm to translate, change altitude, reconfigure its spatial extent, and redirect the viewpoint concurrently, the interface could support navigation through tighter, more irregular, or dynamically changing three-dimensional spaces.
